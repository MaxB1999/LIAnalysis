...
def send_email(post_text: str, subject: str, cfg) -> bool:
    """
    ...
    body_html = (
        f"<html><body>"
        f"<p>Your LinkedIn post for <strong>{datetime.now().strftime('%Y-%m-%d')}</strong> is ready:</p>"
        f"<hr>"
        f"<p style='white-space:pre-wrap;font-family:Georgia,serif;font-size:14px;'>{post_text}</p>"
        f"<hr>"
        f"<p style='color:#888;font-size:12px;'>Generated by LinkedIn Content Generator</p>"
        f"</body></html>"
    )
...
def main() -> None:
    ...
    archive_post(final_post, content_label)
    ...
    email_sent = send_email(final_post, subject, config)
    if not email_sent:
